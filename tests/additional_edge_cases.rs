...
fn test_functional_simulator_boundary_conditions() {
    ...
    simulator
        .memory
        .write_word_init(boundary_address, 0x12345678);
    assert_eq!(
        simulator.memory.read_word(boundary_address),
        Some(0x12345678)
    );
...
fn test_instruction_decode_edge_cases() {
    ...
    assert!(matches!(
        nop,
        vmips_rust::functional_simulator::instructions::Instruction::Nop
    ));
    ...
    assert!(matches!(
        invalid,
        vmips_rust::functional_simulator::instructions::Instruction::InvalidInstruction
    ));
