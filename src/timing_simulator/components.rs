...
use super::config::{CacheConfig, ReplacementPolicy, PrefetchStrategy as ConfigPrefetchStrategy};
...
fn create_prefetcher(config: &CacheConfig) -> Option<Prefetcher> {
    if config.prefetch_enabled {
        ...
        match config.prefetch_strategy {
            ConfigPrefetchStrategy::NextNBlocks(n) => Some(Prefetcher::new(ComponentPrefetchStrategy::NextNBlocks(n))),
            ConfigPrefetchStrategy::AdjacentSets(n) => Some(Prefetcher::new(ComponentPrefetchStrategy::AdjacentSets(n))),
            ConfigPrefetchStrategy::Stride(s) => Some(Prefetcher::new(ComponentPrefetchStrategy::Stride(s))),
...
impl Cache {
    ...
    pub fn new_with_next_level(config: CacheConfig, memory: Memory, next_level: Option<Box<Cache>>) -> Self {
    ...
    pub fn read(&mut self, address: usize) -> Option<(Vec<u8>, usize)> {
        ...
        if let Some(prefetcher) = &self.prefetcher {
            let prefetch_addresses = prefetcher.get_prefetch_addresses(block_addr, self.config.block_size);
...
impl CacheHierarchy {
    pub fn new(memory: Memory, l1d_config: CacheConfig, l1i_config: CacheConfig, 
              l2_config: Option<CacheConfig>) -> Self {
        // Create L2 cache if configured
        let l2_cache = l2_config.map(|config| {
            Box::new(Cache::new(config, memory.clone()))
        });
        
        // Create L1 data cache with L2 as next level
        let l1_data_cache = Cache::new_with_next_level(
            l1d_config,
            memory.clone(),
            l2_cache
        );
    ...
    pub fn update_stats(&mut self) {
        ...
        if total_accesses > 0 {
            self.stats.average_access_time = 
                (l1d_stats.total_access_time + l1i_stats.total_access_time) as f64 / total_accesses as f64;
        }
